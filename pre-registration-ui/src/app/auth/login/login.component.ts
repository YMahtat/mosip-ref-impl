...
export class LoginComponent implements OnInit {
    ...
    loginIdValidator() {
        this.errorLoginInput = undefined;
        ...
        if (modes === "email,mobile") {
            if (
                ...
                this.errorLoginInput = this.validationMessages["invalidInput"];
            }
        } else if (modes === "email") {
            if (!emailRegex.test(this.inputContactDetails)) {
                this.errorLoginInput = this.validationMessages["invalidEmail"];
            }
        } else if (modes === "mobile") {
            if (!phoneRegex.test(this.inputContactDetails)) {
                this.errorLoginInput = this.validationMessages["invalidMobile"];
            }
        }
    }
    ...
    submit(): void {
        ...
        } else if (this.showVerify && this.errorLoginInput === undefined) {
    ...
    verifyInputFieldContact(): void {
        const phoneRegex = this.configService.getConfigByKey(appConstants.CONFIG_KEYS.mosip_regex_phone);
        const emailRegex = this.configService.getConfigByKey(appConstants.CONFIG_KEYS.mosip_regex_email);
        // const emailRegex = '[a-zA-Z0-9].*@[a-zA-Z0-9].*.[a-zA-Z].*';
        if (!this.inputContactDetails.match(new RegExp(phoneRegex)) && !this.inputContactDetails.match(new RegExp(emailRegex))) {
            this.errorLoginInput = true;
        } else {
