...
export class FileUploadComponent implements OnInit, OnDestroy {
        ...
        for (let i of this.activeUsers) {
            ...
            if (i.files) {
                for (let file of i.files.documentsMetaData) {
                    if (file.docCatCode === "POA" || file.docCatCode === "POR") {
        ...
        const noneApplicant = {
            demographicMetadata: {
                [this.name]: [
                    {
                        language: "",
                        value: "Document existant", // value: "None",
                    },
                ],
            },
            preRegistrationId: "",
    ...
    getPiecesJustificativesToDisplay(): any[] {
        return this.LOD.filter(document => this.isToDisplayProofsType(document));
    }

    isToDisplaySameAsProof(document): boolean {
        const isExistingAttachementInSameCatCodePOAForOthersApplicants = this.isExistingAttachementInSamePOAForOthersApplicants('POA', document);
        return (this.isToDisplayProofsType(document) && isExistingAttachementInSameCatCodePOAForOthersApplicants && Boolean(this.allApplicants));
    }

    isExistingAttachementInSamePOAForOthersApplicants(docCatCode: string, document): boolean {
        return document.code === docCatCode && this.allApplicants.filter(app => app.demographicMetadata && app.demographicMetadata.proofOfAddress && app.demographicMetadata.proofOfAddress.docCatCode === document.code).length > 0;
    }
    ...
    isToDisplayProof(documentType): boolean {
        const codesPiecesJustificativesToBring = this.getPiecesJustificativesToBring(this.users[0].request.demographicDetails.identity);
