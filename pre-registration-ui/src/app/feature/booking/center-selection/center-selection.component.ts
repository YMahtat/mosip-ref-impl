...
export class CenterSelectionComponent
    ...
    getCenterSelectionLabels() {
        let factory = new LanguageFactory(this.primaryLang);
        let response = factory.getCurrentlanguage();
        this.errorlabels = response["error"];
        this.centerSelectionLabels = response["centerSelection"];
    }
    ...
    routeNext() {
        ...
        // @HighMinarets : Feature add cas Center without time selection :
        if (this.selectedCentre.centerTypeCode.indexOf('-AVRDV') >= 0) {
            // Navigate to times selcection :
            this.router.navigate(["../pick-time"], {
                relativeTo: this.route,
                queryParams: {regCenter: this.selectedCentre.id},
            });
        } else {
            // Notice that center does not need appoitment and after confirmation book without time selection :
            this.makeBookingForSelectedCenterWithoutTimeSelectionAfterConfirmation();
        }
    }
     ...
     */
    private makeBookingForSelectedCenterWithoutTimeSelectionAfterConfirmation() {
        const data = {
            case: "CONFIRMATION",
            title: "",
            message: this.centerSelectionLabels.confirmationDialogContinueWithoutTimeSelectionQuestionMessage,
            yesButtonText: this.centerSelectionLabels.confirmationDialogContinueWithoutTimeSelectionYesButtonLabel,
            noButtonText: this.centerSelectionLabels.confirmationDialogContinueWithoutTimeSelectionNoButtonLabel,
        };
        const dialogRef = this.dialog.open(DialougComponent, {
            width: "350px",
            data: data,
        ...
        dialogRef.afterClosed().subscribe((selectedOption) => {
            if (selectedOption && (selectedOption !== this.centerSelectionLabels.confirmationDialogContinueWithoutTimeSelectionNoButtonLabel)) {
                // Directly book without time selection :
                this.makeBookingForSelectedCenterWithoutTimeSelection();
            }
        });
    }
     ...
     * TODO @Youssef : à documenter !
     */
    private makeBookingForSelectedCenterWithoutTimeSelection(): void {
        const obj = {
            bookingRequest: [
                new BookingModel(
                    this.preRegId[0],
                    this.selectedCentre.id.toString(),
                    '9999-12-31',
                    "09:00:00",
                    "09:15:00"
                )
