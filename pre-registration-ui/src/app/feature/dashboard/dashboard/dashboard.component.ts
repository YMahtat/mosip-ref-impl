...
export class DashBoardComponent implements OnInit, OnDestroy {
            ...
            (applicants: any) => {
                ...
                if (
                    ...
                    for (
                        ...
                        this.getUserInfo(applicant, applicant.applicationID);
    ...
    getUserInfo(applicant, preRegId) {
        return new Promise((resolve) => {
            this.dataStorageService.getUser(preRegId).subscribe((response) => {
                if (response[appConstants.RESPONSE]) {
                    applicant.request = response[appConstants.RESPONSE];
    ...
    createApplicant(applicants: any, index: number) {
        ...
        let lang = (applicantResponse["demographicMetadata"][this.name]) ?
            applicantResponse["demographicMetadata"][this.name][0]["language"] : 'EMPTY';  // TODO @Youssef : à enlever !
        ...
        const applicant: Applicant = {
            ...
            name: `${applicantResponse["demographicMetadata"]['firstName'][primaryIndex]["value"]} ${applicantResponse["demographicMetadata"]['lastName'][primaryIndex]["value"]}`,
            center: undefined,
            ...
            nameInSecondaryLanguage: (applicantResponse["demographicMetadata"][this.name]) ?
                applicantResponse["demographicMetadata"][this.name][secondaryIndex][
                    "value"
                    ] : 'EMPTY', // TODO @Youssef : à enlever !
            postalCode:
                applicantResponse["demographicMetadata"][
                    appConstants.DASHBOARD_RESPONSE_KEYS.applicant.postalCode
                    ],
        };
        this.dataStorageService.getRegistrationCentersById(
            (applicantResponse && applicantResponse.bookingMetadata ? applicantResponse.bookingMetadata.registration_center_id : undefined),
            this.primaryLangCode
        ).subscribe((data: any) => {
            applicant.center = data && data.response && data.response.registrationCenters && data.response.registrationCenters[0] ? data.response.registrationCenters[0].name : undefined
        });
        return applicant;
    }
    ...
    confirmationDialog(selectedOption: number, applicationID?) {
        let body = {};
        if (Number(selectedOption) === 1) {
            body = {
                case: "CONFIRMATION",
                title: this.secondaryLanguagelabels.title_confirm,
                message: this.secondaryLanguagelabels.deletePreregistration.msg_confirm.replace('{applicationID}', applicationID),
        ...
        const dialogRef = this.openDialog(body, "350px");
        return dialogRef;
    }
    ...
    onDelete(element) {
        ...
        //             if (confirm !== this.secondaryLanguagelabels.button_cancel) {
        ...
        //     }
        // });
        let dialogRef = this.confirmationDialog(1, element.applicationID);
        dialogRef.afterClosed().subscribe((confirm) => {
            if (confirm !== this.secondaryLanguagelabels.button_cancel) {
                this.deletePreregistration(element);
            }
    ...
    getIsExpiredApplicationFromCreationDate(createdDateTime: string): boolean {
        const expirationDate = this.getExpiryDateFromCreationDate(createdDateTime);
